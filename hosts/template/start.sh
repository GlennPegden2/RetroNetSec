...
#Start the emulator
qemu-system-x86_64 -m 16 -cpu 486 -net none  -device ne2k_isa,netdev=lan,irq=10,iobase=0x320,mac=52:54:00:AB:$(printf "%02X" $((RANDOM % 256))):$(printf "%02X" $((RANDOM % 256)))  -netdev vde,id=lan,sock=/tmp/sw1 -hda /root/template.img -parallel none -vnc :0 
